...
public class Application {
    public static void main(String[] args) {
        ...
        for (Name name : names) {
            ...
            Response<Player> playerResponse = blackJackController.createBatting(name, new BattingConverter().convert(battingRequest)); //todo: dto

            players.add(playerResponse.getData());
        }
        ...
        console.displayDealerResult(game.getDealer(), dealerSumResponse.getData());

        // 8. 플레이어 합계 출력
        for (Player player : players) {
            Response<Integer> playerSumResponse = blackJackController.sum(player);
            console.displayPlayerResult(player, playerSumResponse.getData());
        }
        ...
        console.displayDealerProfit(dealerProfitResponse.getData());

        // 10. 플레이어 결과 출력
        for (Player player : players) {
            Response<Integer> playerProfitResponse = blackJackController.playerProfit(player, game.getDealer());
            console.displayPlayerProfit(player, playerProfitResponse.getData());
        }
